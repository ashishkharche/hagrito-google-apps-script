...
function createExpensesCard(opt_prefills, opt_status) {
  ...
  var sheetName = CardService.newTextInput()
    ...
    .setTitle("Sheet Name");
  var createExpensesSheet = CardService.newAction().setFunctionName(
    "createExpensesSheet"
  );
  ...
  newSheetSection.addWidget(
    CardService.newButtonSet().addButton(newSheetButton)
  );
...
function submitForm(e) {
  ...
  try {
    ...
    sheet.appendRow(
      objToArray(res, FIELDNAMES.slice(0, FIELDNAMES.length - 1))
    );
...
function createExpensesSheet(e) {
  ...
  return createExpensesCard(
    prefills,
    "Created and linked the spreadsheet <i>" + sheetName + "</i> for expenses!"
  ).build();
}
